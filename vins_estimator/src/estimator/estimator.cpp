...
void Estimator::clearState()
{
    ...
    for (int i = 0; i < WINDOW_SIZE + 1; i++)
    {
        ...
        Ps[i].setZero();
        Vs[i].setZero();
        Bas[i].setZero();                           //Bas[i] 表示第 i 个相机的加速度偏差
        Bgs[i].setZero();                           //Bgs[i] 则表示第 i 个相机的背景误差
        dt_buf[i].clear();                          //存储与第 i 个相机相关的时间差值数据
...
void Estimator::setParameter()
{
    ...
    ProjectionTwoFrameTwoCamFactor::sqrt_info = FOCAL_LENGTH / 1.5 * Matrix2d::Identity();  //FOCAL_LENGTH是相机焦距
    ProjectionOneFrameTwoCamFactor::sqrt_info = FOCAL_LENGTH / 1.5 * Matrix2d::Identity(); //同一帧时间内，从两个不同相机源获取的数据
...
void Estimator::processMeasurements()
{
    while (1)
    {
        ...
        //map提供了一种键-值对（key-value）的存储方式。map 中的元素按照键的顺序自动排序，并且每个键在 map 中是唯一的。
        pair<double, map<int, vector<pair<int, Eigen::Matrix<double, 7, 1> > > > > feature; //pair 的使用为.first  .second
        vector<pair<double, Eigen::Vector3d>> accVector, gyrVector;
        if(!featureBuf.empty())
        {
            // 当前图像帧特征
            feature = featureBuf.front();
            curTime = feature.first + td;  //时间
            while(1)
            {
...
void Estimator::fastPredictIMU(double t, Eigen::Vector3d linear_acceleration, Eigen::Vector3d angular_velocity)
{
    ...
    latest_time = t;
    ////默认初始值 Q 实部（w）：1.0  虚部（x, y, z）：0.0 ， 其他all zero
    // 前一帧加速度（世界系）
    Eigen::Vector3d un_acc_0 = latest_Q * (latest_acc_0 - latest_Ba) - g;         //加速度从体系（IMU坐标系）转换到世界坐标系，然后减去重力加速度g，得到无重力加速度un_acc_0。
    Eigen::Vector3d un_gyr = 0.5 * (latest_gyr_0 + angular_velocity) - latest_Bg; //对最新的陀螺仪读数和上一次的读数求平均，并减去陀螺仪偏差，得到无偏差角速度un_gyr
    // 更新旋转Q
